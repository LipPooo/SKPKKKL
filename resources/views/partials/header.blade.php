                    ...
                    <div class="p-4 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
                        ...
                        @if($unreadCount > 0)
                            <form action="{{ route('notifications.mark-as-read') }}" method="POST">
                                @csrf
                                <button type="submit" class="text-xs text-blue-600 hover:underline">Tandakan semua dibaca</button>
                            </form>
                        @endif
                    </div>
                    <div class="max-h-96 overflow-y-auto">
                        @forelse(Auth::user()->notifications()->latest()->take(10)->get() as $notification)
                            <a href="{{ route('notifications.read', $notification->id) }}" class="block p-4 hover:bg-gray-50 transition-colors border-b border-gray-50 {{ $notification->read_at ? '' : 'bg-blue-50/30' }}">
                                <div class="flex gap-3">
                                    <div class="w-10 h-10 rounded-full shrink-0 flex items-center justify-center
                                        @if($notification->data['type'] === 'new_request') bg-blue-100 text-blue-600
